...
// Helper function to transform Kitsu API data to our format
function transformKitsuAnimeToAnime(kitsuAnime: any): Anime {
  const attributes = kitsuAnime.attributes;

  // Extract title (use en or en_jp or canonicalTitle as fallback)
  const title =
  ...
  const image =
    ...
    'https://cdn.myanimelist.net/images/anime/1223/96541.jpg'; // Fallback image

  // Map status
  let status = 'Unknown';
  if (attributes.status === 'finished') status = 'Completed';
  else if (attributes.status === 'current') status = 'Ongoing';
  else if (attributes.status === 'upcoming') status = 'Not Yet Aired';
  else status = attributes.status || 'Unknown';

  return {
    id: kitsuAnime.id || '0',
    title,
    image,
    synopsis: attributes.synopsis || 'No description available.',
    type: attributes.subtype?.toUpperCase() || attributes.showType?.toUpperCase() || 'TV',
    status,
    episodes: attributes.episodeCount || 0,
    genres: [], // Will be populated separately
    rating: attributes.averageRating ? (parseFloat(attributes.averageRating) / 10).toFixed(2) : 'N/A',
    aired: attributes.startDate ? new Date(attributes.startDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) : 'Unknown'
  };
}
...
export async function fetchAnimeList(page = 1, limit = 20): Promise<{ data: Anime[], total: number }> {
  ...
  try {
    ...
    // Fetch anime list from Kitsu
    const response = await fetch(
      `${KITSU_API_BASE}/anime?page[limit]=${limit}&page[offset]=${offset}&sort=-userCount`,
      {
        headers: {
    ...
    const animeList = await Promise.all(data.data.map(async (kitsuAnime: any) => {
      const anime = transformKitsuAnimeToAnime(kitsuAnime);

      // Fetch categories/genres for this anime
      try {
        const genresResponse = await fetch(
          `${KITSU_API_BASE}/anime/${kitsuAnime.id}/categories?page[limit]=10`,
          {
            headers: {
              'Accept': 'application/vnd.api+json',
              'Content-Type': 'application/vnd.api+json'
        ...
        if (genresResponse.ok) {
          ...
          anime.genres = genresData.data.map((category: any) => category.attributes.title || '');
        }
      } catch (error) {
        console.error(`Error fetching genres for anime ${kitsuAnime.id}:`, error);
      }
    ...
    return {
      data: animeList,
      total: data.meta?.count || animeList.length
    };
  } catch (error) {
    console.error('Error fetching anime list from Kitsu:', error);
...
export async function fetchAnimeDetails(id: string): Promise<Anime | null> {
  ...
  try {
    ...
    const data = await response.json();
    const anime = transformKitsuAnimeToAnime(data.data);

    // Fetch categories/genres for this anime
    try {
      const genresResponse = await fetch(
        `${KITSU_API_BASE}/anime/${id}/categories?page[limit]=10`,
        {
          headers: {
            'Accept': 'application/vnd.api+json',
            'Content-Type': 'application/vnd.api+json'
      ...
      console.error(`Error fetching genres for anime ${id}:`, error);
    }

    return anime;
  } catch (error) {
    console.error(`Error fetching anime details for ID ${id} from Kitsu:`, error);
    // Fallback to sample data if API fails
    const anime = SAMPLE_ANIME.find(a => a.id === id);
    return anime || SAMPLE_ANIME[0];
  }
...
export async function fetchEpisodes(animeId: string): Promise<Episode[]> {
  ...
  try {
    // Fetch episodes from Kitsu
    const response = await fetch(
      `${KITSU_API_BASE}/anime/${animeId}/episodes?page[limit]=20&sort=number`,
      {
        headers: {
          'Accept': 'application/vnd.api+json',
          'Content-Type': 'application/vnd.api+json'
    ...
    // Get the anime for thumbnail fallback
    const animeResponse = await fetch(
      `${KITSU_API_BASE}/anime/${animeId}`,
      {
        headers: {
          'Accept': 'application/vnd.api+json',
          'Content-Type': 'application/vnd.api+json'
    ...
    if (animeResponse.ok) {
      ...
      fallbackImage =
        animeData.data.attributes.posterImage?.medium ||
        'https://cdn.myanimelist.net/images/anime/1223/96541.jpg';
    }

    // Transform to our Episode format
    const episodes = data.data.map((episode: any) => {
      return {
        id: episode.id,
        title: episode.attributes.canonicalTitle || episode.attributes.titles?.en_jp || `Episode ${episode.attributes.number}`,
        number: episode.attributes.number || 0,
        image: episode.attributes.thumbnail?.original || fallbackImage,
        sources: [{ url: '#', quality: 'HD' }] // Placeholder as Kitsu doesn't provide video sources
      };
    });

    return episodes;
  } catch (error) {
    console.error(`Error fetching episodes for anime ID ${animeId} from Kitsu:`, error);
    // Fallback to sample data if API fails
    return SAMPLE_EPISODES;
  }
}
...
export async function searchAnime(query: string): Promise<Anime[]> {
  ...
  try {
    ...
    const response = await fetch(
      `${KITSU_API_BASE}/anime?filter[text]=${encodeURIComponent(query)}&page[limit]=20`,
      {
        headers: {
          'Accept': 'application/vnd.api+json',
          'Content-Type': 'application/vnd.api+json'
    ...
    console.error(`Error searching anime with query "${query}" from Kitsu:`, error);
    // Fallback to sample data if API fails
    const filteredAnime = SAMPLE_ANIME.filter(anime =>
      anime.title.toLowerCase().includes(query.toLowerCase()) ||
      anime.genres.some(genre => genre.toLowerCase().includes(query.toLowerCase()))
